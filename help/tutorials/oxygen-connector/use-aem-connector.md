...
- Oxygen XML Author 버전 24.1

- Adobe Experience Manager Guides 버전 3.4 이상

- Adobe Experience Manager 버전 6.5(서비스 팩 10, 11, 12 및 13 포함)

- Oxygen XML Author 버전 24.1에서 지원하는 운영 체제
...
1. AEM 인스턴스에 로그인 *\(관리자로서\)* crx 패키지 관리자로 이동합니다. 패키지 관리자에 액세스하기 위한 기본 URL은 입니다.
   ...
   패키지 관리자는 로컬 AEM 설치에서 패키지를 관리합니다. 패키지 관리자 작업에 대한 자세한 내용은 [패키지를 사용하여 작업하는 방법](https://docs.adobe.com/docs/en/aem/6-3/administer/content/package-manager.html) AEM 설명서에서 참조할 수 있습니다.
...
1. 체크 아웃된 파일을 마우스 오른쪽 버튼으로 클릭하고 다음 두 옵션 중에서 선택합니다.
...
1. \(선택 사항\) 체크 인 대화 상자에서 주석을 추가합니다. **버전 설명** 텍스트 상자입니다.
...
1. 클릭 **창** \> **보기 표시** \> **AEM Guides에서 체크 아웃된 파일**.
...
1. 에셋에서 항목을 선택하고 **Oxygen에서 편집** 옵션을 선택합니다.
...
1. 다음에서 **사용 가능** 속성 편집 대화 상자의 열에서 속성을 선택하고 **추가**.
...
문제 : Oxygen XML 작성자에 AEM Guides 패널이 표시되지 않으면 다음 솔루션을 시도해 보십시오.

해결 방법 1: : 1.  Oxygen XML Author에서 플러그인을 활성화합니다.

    **옵션** \> **환경 설정** \> **플러그인**을 클릭하고 **Adobe Experience Manager Guides용 산소 플러그인**을 선택합니다.

1. Oxygen XML 작성자를 다시 시작합니다.


해결 방법 2: : AEM Guides 패널이 표시되지 않으면 AEM Guides 창을 활성화합니다.

    Oxygen XML Author에서 **창** \> **보기 표시** \> **AEM Guides**를 클릭합니다.

해결 방법 3: : Adobe Experience Manager Guides용 Oxygen Plugin을 제거하고 다시 설치합니다.

    - Windows의 경우 **프로그램 추가 또는 제거** 목록에서 플러그인을 제거합니다. 그런 다음 플러그인을 다시 설치합니다.
    
    - Mac에서 Oxygen XML Author의 plugins 폴더에 있는 aem-connector-x.x 폴더에 액세스하여 **Trash**로 이동합니다. 그런 다음 **Trash** 폴더를 비웁니다.


### DITA-OT 변환을 위한 포트 구성

문제 : 플러그인에서 처리한 파일에서 DITA-OT 변환을 실행하면 다음 오류가 발생하여 변환이 실패합니다.

    ![](images/proxy-server-path-error.png)

해결 방법 : 이 문제는 DITA-OT와 플러그인 사이에 프록시 서버를 추가하여 해결했습니다. 이 프록시 서버는 변환을 위해 DITA-OT에서 요청한 모든 파일을 처리하고 공유합니다. 이 서버가 구성된 기본 포트는 다음과 같습니다. `5972`. 다른 서버에 이 포트를 사용하는 경우 프록시 서버에 다른 포트를 지정할 수 있습니다.

    프록시 서버의 기본 포트를 변경하려면 다음 단계를 수행하십시오.
    
    1.  사용자의 홈 디렉터리로 이동합니다.
    
    2.  aem\_connector\_proxy라는 파일을 만듭니다.
    
    3.  텍스트 편집기에서 파일을 열고 파일의 첫 번째 줄에 사용 가능한 포트 번호를 추가합니다.
    
    4.  파일을 저장하고 닫습니다.
    
    5.  Oxygen XML 작성자를 재시작하고 DITA-OT 변환을 실행합니다.
