...
**참고:** 특정 Adobe Experience Manager Guides에 대한 릴리스 정보에서 Oxygen Connector 버전 호환성을 확인합니다.
...
2. 클릭 **다음** Oxygen XML 작성자의 .exe 파일을 사용할 수 있는 위치를 찾습니다.
...
3. Oxygen XML Author의 plugins 폴더에서 aem-connector-x.x 폴더를 복사합니다.
4. aem-connector-x.x-setup 파일을 두 번 클릭하여 설치 관리자를 실행합니다.

5. Oxygen XML 작성자를 시작합니다.
...
### AEM 웹 인터페이스에서 문서 편집 기능을 활성화하는 패키지 설치 {#id182CE0Q0TY4 .section}
...
2. AEM 인스턴스에 로그인 *\(관리자로서\)* crx 패키지 관리자로 이동합니다. 패키지 관리자에 액세스하기 위한 기본 URL은 입니다.
...
3. Oxygen 패키지를 업로드하려면 **패키지 업로드**.
4. 패키지 업로드 대화 상자에서 1단계에서 다운로드한 Oxygen 패키지 파일로 이동한 다음 확인을 클릭합니다.

   패키지가 AEM 인스턴스에 업로드됩니다.

5. 설치 프로세스를 시작하려면 다음을 클릭하십시오. **설치**.

   ![](images/oxygen-package.png)

6. 패키지 설치 대화 상자에서 **설치**.
7. 설치가 완료되면 CRX 패키지 관리자의 왼쪽 상단 모서리에 있는 홈 버튼을 클릭합니다.
8. 에셋 폴더에서 DITA 파일을 선택합니다.
...
### 프로파일링 속성 사용자 지정에 대한 환경 설정 {#id1827K0D0OHT .section}
...
2. 다음에서 **문서 유형 연결** 탭, 선택 **DITA**&#x200B;을 클릭한 다음 을 클릭합니다 **확장**.

   ![](images/document_type_association.png)

3. 다음에서 **클래스 경로** 탭에서 com.adobe.o2.connector를 선택합니다 **ID가 있는 플러그인의 상위 클래스 로더 사용** 드롭다운.

   ![](images/DITA%20Extension.png)

4. 다음에서 **확장** 탭에서 다음 사항을 변경합니다.
5. 
...
6. 클릭 **확인** 모든 대화 상자에서 변경 내용을 저장합니다.

### DITA 맵 확장 구성 { .section}
...
1. Oxygen XML Author에서&#x200B;**옵션** \> **환경 설정**.
2. 다음에서 **문서 유형 연결** 탭, 선택 **DITA 맵**&#x200B;을 클릭한 다음 을 클릭합니다 **확장**.
3. 다음에서 **클래스 경로** 탭에서 com.adobe.o2.connector를 선택합니다 **ID가 있는 플러그인의 상위 클래스 로더 사용** 드롭다운.
4. 다음에서 **확장** 탭에서 다음 사항을 변경합니다.
5. 
...
- **새로 고침**: AEM 저장소에서 파일 및 폴더의 최신 상태를 가져오려면 이 옵션을 선택합니다.
   ...
   **참고:** 파일을 새로 고치기 전에 파일을 저장해야 합니다. 다음을 선택할 때 **새로 고침** 옵션을 선택하면 파일을 새로 고치기 전에 저장하라는 경고가 표시됩니다. 파일을 저장하지 않은 경우 **취소** 그들을 구해
...
- **다음에서 열기**: AEM Guides의 웹 편집기, 맵 대시보드 또는 맵 편집기에서 선택한 파일을 열 수 있습니다. 이러한 옵션에 대한 자세한 내용은 [AEM Guides 편집기에서 파일 열기](#id195GH0V30KX).
...
- **즐겨찾기에 추가**: 폴더에 폴더를 추가합니다. *즐겨찾기* [AEM 안내서] 패널의 폴더입니다. 여기에 작업 폴더를 추가하는 것이 좋습니다. 이렇게 하면 AEM에서 파일과 파일의 상태를 보다 쉽게 동기화할 수 있습니다.
...
### Oxygen XML 작성자에서 파일 열기 {#id195GHJ0A0UB .section}
...
2. 선택 **열기** 컨텍스트 메뉴 아래의 제품에서 사용할 수 있습니다.
...
### 파일 체크 아웃 {#id195HC020TS4 .section}
...
3. \(선택 사항\) 체크 인 대화 상자에서 주석을 추가합니다. **버전 설명** 텍스트 상자입니다.
...
### AEM Guides 보기에서 체크 아웃된 파일 { .section}
...
2. 이 보기에서 파일을 마우스 오른쪽 단추로 클릭하여 다음 옵션을 가져옵니다.
...
### 파일 및 폴더 업로드 {#id195HC03F03J .section}
...
### 즐겨찾기 추가 또는 제거 {#id195HC04405P .section}
...
### 파일 버전 내역 보기 {#id195GI000D5Q .section}

파일의 버전 내역을 보려면 다음 단계를 수행하십시오.

1. AEM Guides 패널에서 파일을 마우스 오른쪽 버튼으로 클릭합니다.

2. 선택 **버전 보기** 컨텍스트 메뉴 아래의 제품에서 사용할 수 있습니다.

   파일의 버전 내역은 버전 대화 상자에 표시됩니다.

   ![](images/version-history.png)


### 파일의 메타데이터 보기 {#id195GHN0H05C .section}

파일의 메타데이터를 보려면 다음 단계를 수행하십시오.

1. AEM Guides 패널에서 파일을 마우스 오른쪽 버튼으로 클릭합니다.

2. 선택 **메타데이터 보기** 컨텍스트 메뉴 아래의 제품에서 사용할 수 있습니다.

   DITA 클래스, 문서 상태, 수정 날짜, 크기, 제목 및 UUID와 같은 파일 메타데이터가 메타데이터 대화 상자에 표시됩니다.
...
2. 검색 쿼리 \(예: `introduction`AEM Guides용 Oxygen Plugin의 검색 막대에서 다음을 수행합니다.
3. 검색 단추를 클릭하거나 Enter 키를 누릅니다.

   결과는 검색 결과 탭에 파일 경로가 있는 목록으로 표시됩니다. 검색 쿼리에 일치하는 결과가 없는 경우에서 결과를 찾을 수 없습니다. &lt;path of=&quot;&quot; the=&quot;&quot; selected=&quot;&quot; folder=&quot;&quot;> 메시지가 표시됩니다.

   ![](images/Search.png)

4. \(선택 사항\) 검색 결과에서 파일을 두 번 클릭하여 Oxygen XML Author에서 엽니다.
5. AEM 저장소 보기로 돌아가려면 다음 중 하나를 수행합니다.
...
2. Oxygen XML 작성자 선택 *&lt;version>* 다음에서 **애플리케이션 실행** 메시지 상자. 다음을 선택할 수 있습니다. **AEM 링크에 대한 내 선택 사항 기억** 기본 설정을 저장하는 옵션입니다.
...
2. 클릭 **Oxygen에서 편집** 옵션을 선택합니다.

   **메모:**

   항목이 체크 아웃되지 않은 경우 먼저 체크 아웃된 다음 편집 모드에서 Oxygen으로 열립니다.

3. Oxygen XML 작성자 선택 *&lt;version>* 다음에서 **애플리케이션 실행** 메시지 상자. 다음을 선택할 수 있습니다. **AEM 링크에 대한 내 선택 사항 기억** 기본 설정을 저장하는 옵션입니다.
4. Oxygen XML 작성자에서 항목을 편집합니다.
5. AEM Guides용 Oxygen Plugin에서 주제를 확인합니다.
...
2. 조건부 속성을 적용할 콘텐츠의 부분을 선택합니다.
3. Oxygen XML 작성자의 속성 패널에서 조건부 속성을 두 번 클릭합니다.

   ![](images/attribute-panel.png)

4. 다음에서 **사용 가능** 속성 편집 대화 상자의 열에서 속성을 선택하고 **추가**.
...
### 누락된 AEM Guides 패널 {#id192BH200ZAX .section}
