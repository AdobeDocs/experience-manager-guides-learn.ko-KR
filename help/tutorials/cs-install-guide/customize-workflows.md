...
| `payloadJson` | JSON 개체 | 다음 값을 지정하십시오. -   `base`: 검토를 위해 보낸 주제가 포함된 상위 폴더의 경로. <br> -   `asset`: 검토를 위해 보낸 주제의 경로. <br> -   `referrer`: 비워 둡니다. |
...
많은 AEM Guides 워크플로우에서 이메일 알림을 사용합니다. 예를 들어 검토 작업을 시작하면 검토자에게 이메일 알림이 전송됩니다. 그러나 이메일 알림이 전송되도록 하려면 AEM에서 이 기능을 활성화해야 합니다. AEM에서 이메일 알림을 활성화하려면 문서를 참조하십시오. [이메일 전송 중](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/implementing/developing/development-guidelines.html#sending-email) AEM 설명서에서 참조할 수 있습니다.

AEM Guides에는 사용자 지정할 수 있는 이메일 템플릿 세트가 포함되어 있습니다. 이러한 템플릿을 사용자 정의하려면 다음 단계를 수행하십시오.

1. 패키지 관리자를 사용하여 다운로드 `/libs/fmdita/mail` 파일.
