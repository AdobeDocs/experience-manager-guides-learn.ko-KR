...
페이지 레이아웃 구성 요소의 설정을 사용하면 페이지의 머리글, 바닥글 및 컨텐츠 영역을 정의하여 페이지 구조를 디자인할 수 있습니다. WYSIWYG 페이지 레이아웃 편집기를 사용하면 앞면 및 뒷면 표지, 챕터, 목차(TOC), 색인, 빈 페이지, 그림 목록(LOF), 표 목록(LOT), 용어집 등 PDF의 다양한 섹션에 대한 페이지 레이아웃을 만들거나 사용자 지정 페이지에 대한 레이아웃을 만들 수 있습니다. PDF 템플릿 설정에서 PDF 내에 서로 다른 섹션이 있는 페이지 레이아웃을 할당하여 PDF 출력을 생성하는 데 사용할 수 있습니다.
...
* **항상 다음에서 챕터 시작:**: 각 챕터가 최종 PDF에 게시되는 방법을 정의할 수 있습니다. 다음 중에서 선택할 수 있습니다. **새 페이지**, **홀수 페이지**, 또는 **짝수 페이지** 옵션. 홀수 페이지에서 새 챕터를 시작하도록 선택하면 홀수 페이지에서 끝나는 챕터 뒤에 빈 페이지가 삽입됩니다. 예를 들어 챕터가 15페이지에 종료되면 게시 프로세스는 빈 16을 삽입합니다<sup>번째</sup> 새 챕터가 17부터 시작되도록 페이지<sup>번째</sup> 페이지를 가리키도록 업데이트하는 중입니다.
...
**다른 섹션에 대한 페이지 레이아웃**: 페이지 레이아웃을 PDF 출력의 다음 섹션과 매핑하도록 선택할 수 있습니다.
* 목차
* 숫자 목록
* 테이블 목록
* 챕터 및 주제
...
PDF 출력에 특정 섹션을 표시하지 않으려면 전환 스위치를 해제하여 비활성화할 수 있습니다.

**챕터 및 항목** 레이아웃은 항상 활성화되어 있고 **용어집** 레이아웃은 기본적으로 항상 비활성화되어 있습니다. 이를 전환할 수 없습니다.

PDF에서 생성되는 이러한 서로 다른 섹션의 순서를 정의할 수도 있습니다. 이러한 페이지의 기본 순서를 변경하려면 페이지 레이아웃의 왼쪽을 마우스로 가리키고 점선 막대 아이콘을 클릭하여 페이지 레이아웃을 원하는 위치에 끌어다 놓습니다.
