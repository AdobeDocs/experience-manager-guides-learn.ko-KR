...
- 요소의 래핑을 해제하면 선택한 텍스트에서 요소 태그를 제거하고 상위 요소와 병합할 수 있습니다. 예를 들어 `p` 다음 내에 있는 요소 `note` 요소, 다음을 unwrap할 수 있습니다 `p` 요소를 사용하여 내에서 직접 텍스트 병합 `note` 요소를 생성하지 않습니다. 항목 이동 경로의 컨텍스트 메뉴에서 요소 래핑 해제 옵션을 사용할 수 있습니다. 요소의 래핑을 해제하려면 항목의 이동 경로에서 요소를 선택한 다음 요소를 마우스 오른쪽 단추로 클릭하여 컨텍스트 메뉴를 연 다음, 마지막으로 요소 래핑 해제를 선택하여 요소를 제거하고 요소의 텍스트를 상위 요소와 병합합니다.
