...
1. 다음 항목 필터링 옵션을 사용하여 목록에 다음 열을 표시하도록 선택할 수도 있습니다.
   ...
   >[!NOTE]
   >
   > 클릭 **새로 고침** 새로운 주제 목록을 가져오고 맵 파일의 변경 사항을 보거나 주제 파일 내의 참조가 업데이트되는지 확인하십시오.
...
1. 다음 항목만 표시하도록 선택할 수도 있습니다. **태그가 없는 파일** 또는 **태그** 관련된 파일을 보려면 필터링하십시오.
   1. 다음 항목 필터링 옵션을 사용하여 메타데이터 목록에 다음 열을 표시하도록 선택할 수도 있습니다.
      ...
      - **태그** \(기본적으로 선택됨\) 파일에 적용된 태그.
         ...
         >[!NOTE]
         >
         > 기본적으로 한 파일에 대해 두 개의 태그를 볼 수 있습니다. 태그를 더 보려면 **더 보기**. 클릭 **간단히 표시** 목록을 다시 계약합니다.
...
1. 다음에서 **필터** 패널 멀티미디어 또는 참조에 사용된 이름별로 목록의 순서를 지정할 수 있습니다.
   ...
   - 주문하시면 **다음에서 사용됨** 이 열에는 멀티미디어가 사용된 참조 이름이 첫 번째 열에 나열되는 반면 멀티미디어 이름이 별도의 행에 있는 다른 열에 나열되는 전치 보기가 표시됩니다. 예를 들어, 다음 스크린샷은 첫 번째 열에 있는 세 개의 참조 \(시트 온도 조정, 시트 온도 표시 변경 및 승무원 영역\) 이름과 멀티미디어 WarmCoolForC.gif가 세 개의 개별 행의 세 번째 열에 표시됩니다.
