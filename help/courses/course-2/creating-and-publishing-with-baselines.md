---
title: 기준선을 사용하여 만들기 및 게시
description: 의 기준선을 사용하여 작성 및 게시 [!DNL Adobe Experience Manager Guides]
exl-id: 3c229c30-f2e0-4fb0-b60c-7bae60ef1a5b
source-git-commit: 1c4d278a05f2612bc55ce277efb5da2e6a0fa9a9
workflow-type: tm+mt
source-wordcount: '739'
ht-degree: 1%

---

# 기준선을 사용하여 만들기 및 게시

기준선을 사용하면 맵 주제와 관련 참조 컨텐츠의 버전을 만들 수 있습니다. 특정 날짜 또는 시간 또는 레이블을 기반으로 할 수 있습니다.

>[!VIDEO](https://video.tv.adobe.com/v/338993?quality=12&learn=on)

## 맵 대시보드에서 기준선 탭 액세스

맵 대시보드에서 기준선에 액세스할 수 있습니다.

1. 저장소 보기에서 맵의 줄임표 아이콘을 선택하여 옵션 메뉴를 연 다음 **맵 대시보드를 엽니다.**

   ![생략 부호-map-dashboard.png](images/ellipsis-map-dashboard.png)
맵 대시보드가 다른 탭에 열립니다.

2. 선택 **기준선**.

   ![baseline-tab.png](images/baseline-tab.png)

베이스라인 탭이 표시됩니다.

## 레이블을 기반으로 기준 요소 만들기

1. 베이스라인 탭에서 **만들기**.

   ![create-baseline.png](images/create-baseline.png)

   새 베이스라인 정보가 표시됩니다. 기본 이름은 작성 날짜를 기반으로 합니다.

2. 필요한 경우 기준선에 새 이름을 지정합니다.

3. &quot;버전 기준 설정&quot; 제목 아래에서 레이블 원을 선택합니다.
   ![set-the-version.png](images/set-the-version.png)

   >[!NOTE]
   >
   >참고: 다음 *레이블이 없으면 최신 버전 사용* 기본적으로 확인란이 선택됩니다. 이 옵션을 선택하지 않고 선택한 레이블이 없는 주제 또는 미디어 파일이 맵에 있으면 베이스라인 작성 프로세스가 실패합니다.

4. 사용할 레이블을 입력합니다.

5. **저장**&#x200B;을 선택합니다.

베이스라인이 생성됩니다. 모든 주제 및 관련 정보의 테이블이 표시됩니다.

### 모든 항목 찾아보기 기능 사용

모든 항목 찾아보기 기능을 사용하면 버전 및 레이블을 비롯한 주제 정보를 볼 수 있을 뿐만 아니라 사용되는 버전을 지정할 수 있습니다. 을 선택하여 액세스할 수 있습니다 **모든 항목 찾아보기** 베이스라인을 만들거나 편집할 때

![browse-all-topics.png](images/browse-all-topics.png)

## 날짜 및 시간을 기반으로 기준 요소 생성

시간 내에 스냅샷인 기준선을 생성할 수도 있습니다.

1. 베이스라인 탭이 열려 있는지 확인하고 생성을 선택합니다.

   ![create-baseline.png](images/create-baseline.png)

2. 버전 기준 설정 제목 아래에서 &quot;버전 기준&quot;에 대한 원을 선택합니다.

   ![version-on.png](images/version-on.png)

3. 달력 아이콘을 선택하고 원하는 날짜 및 시간을 지정합니다.

   ![calendar.png](images/calendar.png)

4. 필요한 경우 기준선에 새 이름을 지정합니다.

5. **저장**&#x200B;을 선택합니다.

베이스라인이 생성됩니다. 모든 주제 및 관련 정보의 테이블이 표시됩니다.

### 기준선에 레이블 추가

모든 맵 컨텐츠에 새 레이블을 일괄적으로 지정할 수 있습니다.

1. 레이블을 추가할 기준선을 선택합니다.

2. 선택 **레이블 추가**.

   ![add-labels.png](images/add-labels.png)

   레이블 추가 대화 상자가 표시됩니다.

3. 지정할 레이블을 입력하고 을(를) 선택합니다 **추가**.

모든 주제에 레이블이 추가되었습니다.

## 기준선을 사용하여 AEM 사이트 출력 생성

1. 맵 대시보드의 출력 사전 설정 탭으로 이동합니다.

2. AEM 사이트 확인란을 선택합니다.

   ![aem-site-checkbox.png](images/aem-site-checkbox.png)

3. **편집**&#x200B;을 선택하십시오.

   ![edit-aem.png](images/edit-aem.png)

   새 페이지가 표시됩니다.

4. 기준 요소 사용 확인란을 선택하고 드롭다운에서 사용할 기준 요소를 선택합니다.

   ![기준선.png](images/baseline.png)

5. 선택 **완료**.

   ![done.png](images/done.png)

6. 선택 **생성**.

   ![generate.png](images/generate.png)

   출력에 베이스라인이 생성되었습니다.

## 생성된 출력 보기

1. 맵 대시보드의 출력 탭으로 이동합니다.

2. 생성 설정 열에서 텍스트를 선택하여 출력을 엽니다.
   ![aem-site-link.png](images/aem-site-link.png)

## 기준선 제거

1. 베이스라인 탭에서 제거할 베이스라인을 선택합니다.

2. 선택 **제거**.

   ![remove-baseline.png](images/remove-baseline.png)

   베이스라인 제거 대화 상자가 표시됩니다.

3. 선택 **제거**.

베이스라인이 제거됩니다.

## 베이스라인 복제

1. 베이스라인 탭에서 복제할 베이스라인을 선택합니다.

2. 선택 **복제**.

   ![duplicate.png](images/duplicate.png)

3. **저장**&#x200B;을 선택합니다.

   ![save.png](images/save.png)

중복 베이스라인이 생성됩니다.

## 베이스라인 수정

베이스라인에 사용되는 항목의 버전을 직접 지정할 수 있습니다.

1. 베이스라인 탭에서 수정할 베이스라인을 선택합니다.
2. **편집**&#x200B;을 선택하십시오.

   ![edit-aem.png](images/edit-aem.png)

3. 선택 **모든 항목 찾아보기**.

   ![browse-all-topics.png](images/browse-all-topics.png)

   주제 테이블과 관련 정보가 표시됩니다.

4. 수정할 항목의 경우 버전 열 아래의 드롭다운에서 원하는 버전을 선택합니다.

   ![version-dropdown.png](images/version-dropdown.png)

5. **저장**&#x200B;을 선택합니다.

변경 사항이 저장되었습니다. 이제 기준선이 지정한 항목의 버전을 사용합니다.

## 사용자 지정된 AEM Site 출력 사전 설정 만들기

[출력] 탭에서 동일한 유형의 기본 출력을 구별하는 것은 어렵습니다. 고유하고 사용자에게 친숙한 이름으로 사용자 지정된 출력 사전 설정을 사용하여 이 문제를 해결할 수 있습니다.

이 경우 기준선을 기반으로 출력 사전 설정을 만들고 있습니다.

1. 맵 대시보드의 출력 사전 설정 탭으로 이동합니다.

2. **만들기**&#x200B;를 선택합니다.

   ![create-output-preset.png](images/create-output-preset.png)

   새 출력 사전 설정 페이지가 표시되고 새 출력이라고 합니다.
3. 설정 이름 필드에 사용자에게 친숙한 이름을 입력합니다.

4. 기준 요소 사용 확인란을 선택하고 드롭다운 메뉴에서 원하는 기준 요소를 선택합니다.

   ![기준선.png](images/baseline.png)

5. 선택 **완료**.

새 출력 사전 설정이 만들어졌고 출력 사전 설정 페이지에 표시됩니다.
